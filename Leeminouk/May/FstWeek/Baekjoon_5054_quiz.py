...
for _ in range(test_case):
    car_pos = 0
    visit_count, store_pos = int(sys.stdin.readline()), list(map(int, sys.stdin.readline().split()))

    car_pos = int(sum(store_pos) / visit_count)
    store_pos.sort()

    print((max(store_pos) - min(store_pos)) * 2)