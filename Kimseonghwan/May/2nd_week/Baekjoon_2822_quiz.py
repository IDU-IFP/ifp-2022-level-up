...
def solution():
    numDict = {
        1: int(input()), 2: int(input()), 3: int(input()), 4: int(input()), 5: int(input()), 6: int(input()), 7: int(input()), 8: int(input())
    }
    sum = cnt = 0
    resultList = list()
    for i, j in sorted(numDict.items(), reverse=True, key=lambda x: x[1]):
        sum += j
        resultList.append(i)
        if cnt == 4:
            print(sum)
            for k in sorted(resultList):
                print(k, end=" ")
            return
        cnt += 1
