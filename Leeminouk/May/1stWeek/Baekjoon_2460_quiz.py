...
def mostCrowedStation():
    total_people = 0

    for _ in range(TOTAL_STATION):
        board, alight = map(int, sys.stdin.readline().split())

        total_people += eachStation(alight, board)

        mostCrowed(total_people)

def eachStation(alight, board):
    return alight - board

def mostCrowed(total):
    global most_crowed

    if total > most_crowed:
        most_crowed = total
